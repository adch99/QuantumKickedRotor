...
def plot(avgs, varis, p):
    ...
    ax2.set_xlabel("n")
    title = f"Kicked Quantum Rotor [K={K}, TAU={TAU}, DIM={DIM}," \
            f" T={TIMESPAN}, DELTAK={DELTAK}, DELTATAU={DELTATAU}]"
    fig.suptitle(title)
    plt.tight_layout()
    filename = f"plots/perturbedquantumkickedrotor_T"\
                f"{TIMESPAN}DIM{DIM}DK{DELTAK}DT{DELTATAU}"\
                f"_{date.today()}.png"
